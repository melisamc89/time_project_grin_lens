            ...
            if beh_index == 0:
                ...
                for band_name, (low, high) in bands.items():
                    ...
                    for ch in range(n_channels):
                        ...
                        # Filter
                        filtered = bandpass_filter(lfp_ch, low, high, fs, order=2)
                        ...
                        # Compute phase-locking stats
                        pl_mean = circmean(pl_neuron_phases, high=np.pi, low=-np.pi)
                        pl_std = circstd(pl_neuron_phases, high=np.pi, low=-np.pi)

                        pl_mean_per_channel.append(pl_mean)
                        pl_std_per_channel.append(pl_std)

                    # Save list of PL stats for all channels
                    pl_bands.append(pl_mean_per_channel)  # shape: [n_channels]
                    pl_bands_std.append(pl_std_per_channel)  # shape: [n_channels]
